...
from lib.add_window import Add_Window_Horizon,Add_Window_Horizon_Token
...
def get_metra_la_loader(args, normalizer='std'):
    data_dict = load_st_dataset(args.dataset)
    ...
    train_loader = data_loader(data_dict['x_train'], data_dict['y_train'], args.batch_size, shuffle=True, drop_last=True)
    val_loader = data_loader(data_dict['x_val'], data_dict['y_val'],args.batch_size, shuffle=True, drop_last=True)
    test_loader = data_loader(data_dict['x_test'], data_dict['y_test'], args.batch_size, shuffle=True, drop_last=False)
...
def get_topo_dataloader(args, normalizer = 'std', single=False):
    #load raw st dataset
    data = load_st_dataset(args.dataset)        # B, N, D
    #normalize st data
    data, scaler = normalize_dataset(data, normalizer, args.column_wise)
    ...
    x, y = Add_Window_Horizon_Token(args.dataset, data, args.lag, args.horizon, single) # x: (193, 7, 100, 1); y: (193, 7, 1) # correct!
    print('x:', x.shape)
    print('y:', y.shape)
